...
export default function Home() {
  ...
  // Replace the useEffect for loading symbols with:
  useEffect(() => {
    const loadSymbols = async () => {
      try {
        ...
        setAllCoins([
          ...
          // Add more fallback pairs as needed
        ]);
      }
    };
    loadSymbols();
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Advanced AI-powered cryptocurrency trading assistant"
        />
      ...
      <main className="min-h-screen bg-gray-950 text-white">
        ...
        <nav className="bg-gray-900 border-b border-gray-800 px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center sticky top-0 z-20">
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            >
              <IoMdRefresh
                className={`${lastUpdated ? "animate-spin-once" : ""}`}
              />
        ...
        <div className="flex">
          ...
          <div
            className={`lg:col-span-1 bg-gray-900 border border-gray-800 p-4 sm:p-5 shadow-lg fixed lg:static z-10 h-[calc(100vh-64px)] lg:h-auto w-3/4 sm:w-1/2 md:w-1/3 lg:w-auto transition-all duration-300 ease-in-out transform ${
          ...
          >
            ...
            <div className="space-y-1 sm:space-y-2 h-[calc(100%-180px)] sm:h-[1000px] overflow-y-auto custom-scrollbar">
              {filteredCoins
                .slice(0, showAllCoins ? undefined : 100)
                .map((coin) => (
                  <div
                    key={coin.symbol}
                    onClick={() => {
                    ...
                    className={`flex items-center justify-between p-2 sm:p-3 rounded-lg cursor-pointer transition-all duration-200 hover:bg-gray-800/70 ${
                      symbol === coin.symbol
                        ? "bg-teal-900/30 border border-teal-800/50 shadow-md"
                        : "bg-gray-800/50"
                    }`}
                  >
                    <div className="flex items-center gap-2 sm:gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400 truncate max-w-[100px] sm:max-w-[120px]">
                          {coin.name}
                        </div>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                  {["BTC", "ETH", "SOL", "XRP"].map((coin) => {
                    ...
                    return (
                      <div
                        key={coin}
                        className="bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-700 hover:shadow-lg transition-all duration-200 hover:scale-[1.02]"
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="font-bold text-sm sm:text-base text-gray-300">
                              {coin}/USDT
                            </div>
